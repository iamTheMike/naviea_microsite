...
export default function WishResult({
  ...
  useEffect(() => {
    ...
    if (user?.wish.length < 100) {
      ...
      if (user?.photoFileIg?.size < 650000) {
  ...
  useEffect(() => {
    if(user?.wish.length < 50){
      setFontsize(55);
    }else if (user?.wish.length < 100) {
      setFontsize(18);
    }else if  (user?.wish.length < 150) {
      setFontsize(14);
    }else if  (user?.wish.length < 200) {
      setFontsize(12);
    }else if  (user?.wish.length < 250) {
      setFontsize(11);
    }else{
      setFontsize(10);
    }   
    ...
    <Box className="flex flex-col   pb-20  ">
      ...
      <Box className="fixed  bottom-[999rem] ">
        ...
        <Box ref={boxRef2}>
          <div
            style={{
              backgroundImage: `url(${POSTCARD_IG_01.src})`,
          ...
          >
            {/* show case */}
            <div className="absolute top-[310px] left-1/2 translate-x-[-50%] h-[330px]  w-[40%] flex justify-center items-center  ">
          ...
          >
            ...
            <Box className="flex flex-col z-50 items-center justify-center  ">
              <Image
                ...
                height={250}
                alt="wish"
                className="object-contain rounded-2xl border-grey border-[1px]"
              />
            </Box>
          </Box>

          <Box className="flex flex-row w-full justify-around z-50 p-5 ">
            <Button
              sx={{
                padding: "0 !important",
              }}
